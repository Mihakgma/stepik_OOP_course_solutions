...
В методе post параметр request - это словарь с ключами 'login' и 'password'
...
class LengthValidator:
    ...
    def __call__(self, *args, **kwargs):
        string = args[0]
        len_min, len_max = self.min_length, self.max_length
        return isinstance(string, str) and len_min <= len(string) <= len_max
...
class CharsValidator:
    ...
    def __call__(self, *args, **kwargs):
        string = args[0]
        chars = self.chars
        return isinstance(string, str) and all([True if i in chars else False for i in string])
...
if __name__ == '__main__':
    ...
    print(allowed_chars)
    lg = LoginForm("Вход на сайт", validators=[LengthValidator(3, 50), CharsValidator(allowed_chars)])
    lg.post({"login": "root", "password": "panda"})
    print(*[(k, v) for (k, v) in lg.__dict__.items()], sep='\n')
    if lg.is_validate():
        print("Дальнейшая обработка данных формы")
