...
buffer = Data(строка с данными, IP-адрес назначения)
...
send_data(buffer) - для отправки информационного пакета buffer (объекта класса Data) с указанным IP-адресом получателя
...
buffer - передаваемые данные (строка);
...
class Server:
    ...
    def send_data(self, data):
        """для отправки информационного пакета buffer (объекта класса Data)
...
class Router:
    ...
    def __init__(self):
        ...
        if len(self.LINKED_SERVERS):
            self.linked_servers = self.LINKED_SERVERS
        else:
            self.linked_servers = []
        if len(self.BUFFER):
    ...
    @classmethod
    def unlink_cls(cls, ip):
        cls.LINKED_SERVERS.remove(ip)
    ...
    def unlink(self, server):
        ...
        # self.unlink_cls(server.ip)

    @classmethod
    def clear_buffer_cls(cls):
        cls.BUFFER = []
...
if __name__ == '__main__':
    ...
    print(router.__dict__)
    router.send_data()
    print(f'АТТРИБУТ КЛАССА РОУТЕР LINKED SERVERS: <{router.LINKED_SERVERS}>')
    router.unlink(server=sv)
    print(router.__dict__)
