...
https://stepik.org/lesson/701996/step/5?unit=702097
"""
...
class DictShop(dict):
    def __init__(self, thing={}):
        if type(thing) != dict:
            raise TypeError('аргумент должен быть словарем')
        if len(thing):
            [self.is_key_valid(k) for k in thing]
    ...
    def is_key_valid(self, x):
        name_cls = Thing
        if type(x) != name_cls:
            raise TypeError(f'ключами могут быть только объекты класса {name_cls}')
        if x not in self:
            return True
        return False
...
if __name__ == '__main__':
    ...
    print(*[(hash(_), _.name, _.price, _.weight, _) for _ in dict_things], sep='\n')

    print(f"Количество ключей в словаре (интернет-магазин) составило <{len(dict_things)}>")
    # dict_things[1] = th_1 # исключение TypeError
