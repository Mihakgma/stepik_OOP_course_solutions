...
add_thing(self, thing) - добавление нового предмета в рюкзак (добавление возможно,
если суммарный вес (max_weight) не будет превышен, иначе добавление не происходит);
...
class Bag:
    ...
    def __init__(self, max_weight: int=1000):
    ...
    def check_thing(self, thing):
        return self.max_weight >= thing.weight + self.get_total_weight()
...
if __name__ == '__main__':
    ...
    bag.add_thing(Thing("Бумага", 100))
    w = bag.get_total_weight()
    print(f'Total weight of <{bag}> object is: <{w}> kilograms')
    for t in bag.things:
        print(f"{t.name}: {t.weight}")
